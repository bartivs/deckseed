...
- A written proposal companion has its own approval gate: agree on audience, continuity with earlier work, document outline, figures, and decision ask before creating or materially changing `proposal.md`.
...
- `export-proposal-pdf` to create a printable written proposal companion.
...
- Optional written proposal source lives in `presentations/<slug>/proposal.md`; generated `proposal.pdf` and `proposal.sha256` remain beside it and ignored.
...
- Run `npm run proposal -- <slug>` after changing an approved `proposal.md`, its local proposal assets, or the associated deck content.
- The builders stamp source hashes into `index.html` or `proposal.sha256`; validation rejects stale output.
...
After building, capture representative slides with a headless browser. Check at least a title slide, card grid, diagram, table, and code/dense slide. For an optional proposal PDF, render its pages with Poppler and inspect at least the first page, densest table, every figure page, and final page.
