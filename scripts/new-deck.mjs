...
if (!slug || !/^[a-z0-9][a-z0-9-]*$/.test(slug)) {
  console.error("Usage: npm run new -- <slug> [--title \"Title\"] [--languages en,es] [--default-language auto] [--themes midnight,paper] [--default-theme auto] [--force]");
  process.exit(1);
}
...
const themeIds = [...new Set((valueAfter("--themes") ?? "midnight,paper").split(",").map((value) => value.trim()).filter(Boolean))];
const defaultTheme = valueAfter("--default-theme") ?? "auto";
...
if (themeIds.length === 0 || themeIds.some((id) => !(id in themeLibrary))) {
  console.error(`--themes must contain built-in theme IDs: ${Object.keys(themeLibrary).join(", ")}`);
  process.exit(1);
}
if (defaultTheme !== "auto" && !themeIds.includes(defaultTheme)) {
  console.error("--default-theme must be auto or one of the configured themes");
  process.exit(1);
}
...
const themes = Object.fromEntries(themeIds.map((id) => [id, themeLibrary[id]]));
const config = {
  id: slug,
